...
export default function TrackOrder() {
  ...
  const handleTrack = (e: React.FormEvent) => {
    ...
    if (!/^PLZ-\d{3}$/.test(trimmed)) {
      setError(true);
      setErrorMessage("Format invalide. Exemple : PLZ-042");
      return;
    }
    setError(false);
    navigate(`/store/zara-maroc/commande/${trimmed}`); // TODO: dynamic slug
  };
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4">
        ...
        >
          ...
          <form onSubmit={handleTrack} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full h-14 bg-[#2563EB] text-white rounded-lg font-medium text-[16px] hover:bg-[#1d4ed8] transition-colors"
            >
              Suivre ma commande
            </button>
          </form>
          ...
          <div className="bg-[#EFF6FF] rounded-xl p-5 border border-[#2563EB]/20">
            ...
            <p className="text-[13px] text-[#78716C] leading-relaxed">
              Votre numéro de commande vous a été fourni lors de la confirmation. Il commence
              par <span className="font-medium text-[#2563EB]">PLZ-</span> suivi de 3 chiffres.
            </p>
          </div>
