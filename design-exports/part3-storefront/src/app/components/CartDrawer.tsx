        ...
        <Drawer.Content className="fixed bottom-0 left-0 right-0 bg-white rounded-t-2xl z-50 max-h-[90vh] flex flex-col">
          <div className="flex-shrink-0">
            ...
            <div className="flex items-center justify-between px-4 pb-4 border-b border-[#E2E8F0]">
              <div>
                <Drawer.Title className="font-bold text-[18px]">Mon panier</Drawer.Title>
                <Drawer.Description className="text-[13px] text-[#78716C]">{items.length} articles</Drawer.Description>
              </div>
              {/* FIX-04: close button w-8 h-8 → w-11 h-11 */}
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4">
            ...
            {items.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full py-16 text-center px-4">
                ...
                <p className="font-medium text-[16px] text-[#1C1917] mb-1">Votre panier est vide</p>
                <p className="text-[14px] text-[#78716C]">Ajoutez des produits pour commencer</p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex gap-3 mb-3">
                      ...
                      <div className="flex-1 min-w-0 flex flex-col">
                        <h3 className="font-medium text-[14px] mb-1 line-clamp-2">{item.name}</h3>
                        <span className="font-bold text-[16px] text-[#2563EB] mt-auto">{item.price} MAD</span>
                      </div>
                      {/* FIX-04: trash button w-8 h-8 → w-11 h-11 */}
                      <button
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-[#E2E8F0]">
                      <div className="flex items-center gap-3">
                        {/* FIX-04: minus button w-9 h-9 → w-11 h-11 */}
                        <button
                        ...
                        {/* FIX-04: plus button w-9 h-9 → w-11 h-11 */}
                        <button
                          onClick={() => updateQuantity(item.id, item.quantity + 1)}
                          className="w-11 h-11 flex items-center justify-center border-2 border-[#E2E8F0] rounded-lg hover:border-[#2563EB] transition-colors"
                        >
                      ...
                      <div className="text-right">
                        ...
                        <div className="font-bold text-[16px]">{item.price * item.quantity} MAD</div>
                      </div>
                    </div>
                  </motion.div>
                ))}
          ...
          <div className="flex-shrink-0 border-t border-[#E2E8F0] p-4 space-y-3">
            <div className="space-y-2">
              ...
              <div className="flex justify-between pt-2 border-t border-[#E2E8F0]">
                ...
                <span className="font-bold text-[20px]">{finalTotal} MAD</span>
              </div>
            </div>
            {/* FIX-12: disabled when cart is empty */}
            <button
            ...
            <button onClick={onClose} className="w-full text-[14px] text-[#78716C] py-2">
