...
export default function IssueSuccessPage({ params }: { params: { id: string } }): JSX.Element {
  const t = useTranslations('driver.issue.success');
  const reference = params.id.slice(0, 8).toUpperCase();
  ...
  return (
    <main className="min-h-screen bg-[#FAFAF9] flex flex-col items-center justify-center px-8">
      ...
      <h1 className="text-[22px] font-bold text-[#1C1917] mt-5">{t('title')}</h1>
      <p className="text-sm text-[#78716C] mt-2 text-center">{t('body')}</p>

      <div className="w-full mt-6 bg-white rounded-2xl shadow-sm p-4 space-y-3">
        <div className="flex items-center gap-3">
          <Clock className="w-5 h-5 text-[#78716C]" />
          <span className="text-sm text-[#1C1917]">{t('reference', { id: reference })}</span>
        </div>
        <div className="flex items-center gap-3">
          <MessageCircle className="w-5 h-5 text-[#78716C]" />
          <span className="text-[13px] text-[#78716C]">{t('responseTime')}</span>
        </div>
      </div>
